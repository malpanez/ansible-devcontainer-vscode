...
**Renovate** (self-hosted, via GitHub Actions) manages dependency updates.
...
- **Weekly PRs** (Mondays, targeting `develop`, auto-merged on green) for:
  - Python packages (pyproject.toml + uv.lock) and the derived
    `requirements*.txt` exports (regenerated by a `uv export` postUpgradeTask)
  - GitHub Actions (SHA-pinned) and Docker base-image digests
  - The `ARG`-pinned CLI tools (terraform, terragrunt, tflint, sops, age,
    tectonic, aws-cli, uv) via regex custom managers; for age/tectonic/
    aws-cli a postUpgradeTask reruns `scripts/refresh-tool-pins.py
    --sync-hashes` to recompute the download SHA-256 Renovate can't
...
- Workflow: [`.github/workflows/renovate.yml`](workflows/renovate.yml)
  (scheduled Monday + `workflow_dispatch`; a `force` input ignores the
  schedule to create PRs on demand)
...
3. **Update Renovate coverage**
   ...
   - If the tool ships no upstream checksum manifest, add it to
     `scripts/refresh-tool-pins.py` and to the hash-sync postUpgradeTask
